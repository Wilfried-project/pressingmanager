...
const STATUS_COLORS: Record<string, { bg: string; text: string; bar: string }> = {
  ...
  annule:     { bg: 'bg-red-50',     text: 'text-red-700',     bar: 'bg-red-500' },
}
              ...
              {viewOrder.status === 'pret' && (
                <div className="mt-3 p-3 bg-emerald-50 border border-emerald-200 rounded-lg flex items-center gap-2">
                  ...
                  <p className="text-xs text-emerald-800 font-medium">
                    Commande prete - le client a ete notifie automatiquement si WhatsApp configure
                  </p>
                </div>
              )}
            </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {viewOrder.clothes.map((cloth) => (
                  <div key={cloth.id} className="flex items-center justify-between gap-3 p-3 bg-surface-container-low rounded-xl">
                    <div className="flex items-center gap-3 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        <p className="text-sm font-bold text-on-surface capitalize truncate">
                          {cloth.quantity}x {cloth.type} {cloth.color ? `(${cloth.color})` : ''}
                        </p>
                        <p className="text-xs text-on-surface-variant truncate">
                          {cloth.service?.replace(/_/g, ' ')} {cloth.brand ? `- ${cloth.brand}` : ''}
                        </p>
                      </div>
                    </div>
                    <div className="text-right shrink-0">
                      <p className="text-sm font-bold text-primary">
                        {((cloth.price || 0) * (cloth.quantity || 1)).toLocaleString('fr-FR')} XOF
                      </p>
                    </div>
                  </div>
                ))}
